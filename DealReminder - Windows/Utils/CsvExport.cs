...
namespace DealReminder_Windows.Utils
{
    ...
    public class CsvExport
    {
        ...
        /// The current row
        /// </summary>
        public Dictionary<string, object> CurrentRow => _rows[_rows.Count - 1];
        ...
        private IEnumerable<string> ExportToLines()
        {
            ...
            // The header
            yield return string.Join(columnSeparator, _fields);
            ...
            foreach (Dictionary<string, object> row in _rows)
            {
                ...
                yield return string.Join(columnSeparator, _fields.Select(field => MakeValueCsvFriendly(row[field], columnSeparator)));
            }
        }
        ...
        {
            File.WriteAllLines(path, ExportToLines(), Encoding.GetEncoding("iso-8859-1"));
        }
        ...
        public byte[] ExportToBytes()
        {
            ...
            var data = Encoding.GetEncoding("iso-8859-1").GetBytes(Export());
            return Encoding.GetEncoding("iso-8859-1").GetPreamble().Concat(data).ToArray();
        }
    }
}
